...
def nasnetamobile(num_classes=1000, pretrained='imagenet'):
    ...
    if pretrained:
        ...
        if num_classes != settings['num_classes']:
            print("Not loading last layer weights")

        checkpoint = model_zoo.load_url(settings['url'], map_location = "cpu")
        # both 'imagenet'&'imagenet+background' are loaded from same parameters
        model = NASNetAMobile(num_classes=num_classes)
        model_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in checkpoint.items() if (k in model_dict and checkpoint[k].shape == model_dict[k].shape)}
